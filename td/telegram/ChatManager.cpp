...
td_api::object_ptr<td_api::accessHash> ChatManager::get_channel_access_hash_object(ChannelId channel_id,
  ...
  return td_api::make_object<td_api::accessHash>(
      channel_id.get(), td_api::make_object<td_api::accessHashTypeChannel>(), c->access_hash);
}
...
void ChatManager::memory_stats(vector<string> &output) {
  output.emplace_back("Basic groups: (WaitFreeHashMap)");
  output.emplace_back("Basic group fulls: (WaitFreeHashMap)");
  output.emplace_back(PSTRING() << "Unknown basic groups: " << unknown_chats_.size());
  output.emplace_back("Basic group full file source IDs: (WaitFreeHashMap)");
  output.emplace_back("Min channels: (WaitFreeHashMap)");
  output.emplace_back("Supergroups: (WaitFreeHashMap)");
  output.emplace_back("Supergroup fulls: (WaitFreeHashMap)");
  output.emplace_back(PSTRING() << "Unknown supergroups: " << unknown_channels_.size());
  output.emplace_back("Supergroup full file source IDs: (WaitFreeHashMap)");
  output.emplace_back(PSTRING() << "Load basic group from database queries: " << load_chat_from_database_queries_.size());
  output.emplace_back(PSTRING() << "Loaded from database basic groups: " << loaded_from_database_chats_.size());
  output.emplace_back(PSTRING() << "Unavailable basic group fulls: " << unavailable_chat_fulls_.size());
  output.emplace_back(PSTRING() << "Load supergroup from database queries: " << load_channel_from_database_queries_.size());
  output.emplace_back(PSTRING() << "Loaded from database supergroups: " << loaded_from_database_channels_.size());
  output.emplace_back(PSTRING() << "Unavailable supergroup fulls: " << unavailable_channel_fulls_.size());
  output.emplace_back("Linked channel IDs: (WaitFreeHashMap)");
  output.emplace_back("Restricted supergroup IDs: (WaitFreeHashSet)");
}
