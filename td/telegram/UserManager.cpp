...
td_api::object_ptr<td_api::accessHash> UserManager::get_user_access_hash_object(UserId user_id,
  ...
  return td_api::make_object<td_api::accessHash>(
      user_id.get(), td_api::make_object<td_api::accessHashTypeUser>(), u->access_hash);
}
...
void UserManager::memory_stats(vector<string> &output) {
  output.emplace_back("Users: (WaitFreeHashMap)");
  output.emplace_back("User fulls: (WaitFreeHashMap)");
  output.emplace_back("User photos: (WaitFreeHashMap)");
  output.emplace_back(PSTRING() << "Unknown users: " << unknown_users_.size());
  output.emplace_back("Pending user photos: (WaitFreeHashMap)");
  output.emplace_back("User profile photo file source IDs: (WaitFreeHashMap)");
  output.emplace_back(PSTRING() << "My photo file IDs: " << my_photo_file_id_.size());
  output.emplace_back(PSTRING() << "Load user from database queries: " << load_user_from_database_queries_.size());
  output.emplace_back(PSTRING() << "Loaded from database users: " << loaded_from_database_users_.size());
  output.emplace_back(PSTRING() << "Unavailable user fulls: " << unavailable_user_fulls_.size());
  output.emplace_back(PSTRING() << "Uploaded profile photos: " << being_uploaded_profile_photos_.size());
  output.emplace_back(PSTRING() << "Imported contacts: " << imported_contacts_.size());
  output.emplace_back(PSTRING() << "Load contacts queries: " << load_contacts_queries_.size());
  output.emplace_back("Restricted user IDs: (WaitFreeHashSet)");
  output.emplace_back(PSTRING() << "Imported contact user IDs: " << imported_contact_user_ids_.size());
  output.emplace_back("Secret chats: (WaitFreeHashMap)");
}
